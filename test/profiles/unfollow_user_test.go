...
func TestGivenUserIsFollowedWhenUnfollowUserShouldReturnProfileWithFollowingFalse(t *testing.T) {
	...
	_, err = FollowUserAndDecode(followee.User.Username, follower.User.Token)
	if err != nil {
		t.Fatal(err)
	}

	profile, err := UnfollowUserAndDecode(followee.User.Username, follower.User.Token)
	if err != nil {
		t.Fatal(err)
	}

	gotProfile, err := GetProfileAndDecode(followee.User.Username, &follower.User.Token)
	if err != nil {
		t.Fatal(err)
	}

	if profile.Profile.Username != gotProfile.Profile.Username {
		t.Fatalf("got %s, want %s", profile.Profile.Username, gotProfile.Profile.Username)
	}

	if profile.Profile.Bio != gotProfile.Profile.Bio {
		t.Fatalf("got %s, want %s", profile.Profile.Bio, gotProfile.Profile.Bio)
	}

	if profile.Profile.Image != gotProfile.Profile.Image {
		t.Fatalf("got %s, want %s", profile.Profile.Image, gotProfile.Profile.Image)
	}

	if profile.Profile.Following != gotProfile.Profile.Following {
		t.Fatalf("got %t, want %t", profile.Profile.Following, gotProfile.Profile.Following)
	}
...
func TestGivenUserIsNotFollowedWhenUnfollowUserShouldReturnProfileWithFollowingFalse(t *testing.T) {
	followeeClient := utils.NewApiClient()
...
func TestGivenInvalidAccessTokenWhenUnfollowUserShouldReturnUnauthorized(t *testing.T) {
	followeeClient := utils.NewApiClient()
	...
	response, err := UnfollowUser(followee.User.Username, "invalidToken")
	if err != nil {
		t.Fatal(err)
	}
...
func TestGivenFolloweeIsNotFoundWhenUnfollowUserShouldReturnNotFound(t *testing.T) {
	followeeUsername := fmt.Sprintf("%s%s", utils.TestPrefix, faker.Username())
	...
	response, err := UnfollowUser(followeeUsername, follower.User.Token)
	if err != nil {
		t.Fatal(err)
	}
